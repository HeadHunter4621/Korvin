...
class Module(
    ...
    private val angleMotorEncoder: CANcoder,
    private val andleMotorEncoderOffset: Double,
    ...
    init{
        ...
        // Drive Motors
        driveConfig.inverted(DrivetrainConstants.driveMotorInverted)
        driveConfig.idleMode(SparkBaseConfig.IdleMode.kBrake)
        driveConfig.smartCurrentLimit(DrivetrainConstants.driveCurrentLimit)
        ...
        // Angle Motors
        angleConfig.inverted(DrivetrainConstants.angleMotorInverted)
        angleConfig.idleMode(SparkBaseConfig.IdleMode.kBrake)
        angleConfig.smartCurrentLimit(DrivetrainConstants.angleCurrentLimit)
    ...
    fun getAngle(): Rotation2d {
        if(inverted) {
            angleMotor.encoder.setPosition(angleMotorEncoder.absolutePosition.valueAsDouble * DrivetrainConstants.ANGLE_ENCODER_MULTIPLY - andleMotorEncoderOffset)
        }else{
            angleMotor.encoder.setPosition((-(angleMotorEncoder.absolutePosition.valueAsDouble * DrivetrainConstants.ANGLE_ENCODER_MULTIPLY - andleMotorEncoderOffset)))
        }
    ...
    fun getGoal():SwerveModuleState { return SwerveModuleState(goal.speedMetersPerSecond, Rotation2d(goal.angle.radians)) }
    ...
    fun getEncoderHealth(): Double { return angleMotorEncoder.magnetHealth.valueAsDouble }
...
object Drivetrain: SubsystemBase() {
    ...
    private var modules: Array<Module>
    ...
    init {
        ...
        Drivetrain.defaultCommand = DrivetrainCommand(true)

    }


    // "Fix this nonsense" -Whoever made the original code
    private fun createModule(driveMotor: SparkMax, angleMotor:SparkMax, moduleData: SwerveModuleData): Module {
    ...
    fun getGoals(): Array<SwerveModuleState> { return arrayOf(modules[0].getGoal(), modules[1].getGoal(), modules[2].getGoal(), modules[3].getGoal()) }
